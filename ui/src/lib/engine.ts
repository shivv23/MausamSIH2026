// Mausam 2.0 — client-side personalization engine.
// Mirrors backend/app/services (impact_models.py, ranking.py, homepage.py) and
// providers/mock.py so the prototype behaves like the real API contract.
...
export interface Homepage {
  ...
  myDay: MyDayItem[];
  freshness: string;
  providers: { name: Source; status: 'ok' | 'degraded' | 'down'; latencyMs: number }[];
}
...
export const PERSONAS: Record<
  PersonaKey,
  { label: string; labelHi: string; icon: string; color: string; soft: string; desc: string; descHi: string }
> = {
...
export const SCENARIOS: ScenarioDef[] = [
  {
    ...
    emoji: '☀️',
    condition: 'sunny',
    desc: 'Baseline — everything favourable',
    params: { aqi: 58, uv: 6, precip: 0, rainProb: 12, wind: 8, temp: 26, humidity: 48, pollen: 4 },
  },
  {
    key: 'clean_air_morning',
    label: 'Clean-air morning',
    labelHi: 'स्वच्छ हवा सुबह',
    emoji: '🌤️',
    condition: 'partly',
    desc: 'Rare AQI 38 — nudge outdoor activity',
    params: { aqi: 38, uv: 3, precip: 0, rainProb: 5, wind: 6, temp: 21, humidity: 55, pollen: 2 },
  },
  {
    key: 'aqi_spike',
    label: 'AQI spike',
    ...
    warning: {
      ...
      source: 'CPCB',
      event: 'Air Quality',
      headline: 'Air quality Very Poor — GRAP Stage III',
      headlineHi: 'वायु गुणवत्ता बहुत खराब — GRAP चरण III',
      ...
      actionsHi: ['बाहर परिश्रम से बचें', 'बाहर N95 पहनें', 'घर पर प्यूरीफ़ायर चलाएँ'],
      validHours: 18,
    },
  },
  {
    ...
    warning: {
      ...
      source: 'IMD',
      event: 'Heat Wave',
      headline: 'Heat wave warning — 42 °C, 5 °C above normal',
      headlineHi: 'लू की चेतावनी — 42 °C, सामान्य से 5 °C अधिक',
      ...
      actionsHi: ['12–4 बजे अंदर रहें', 'हर घंटे पानी / ORS', 'बुज़ुर्गों का ध्यान रखें'],
      validHours: 30,
    },
  },
  {
    ...
    warning: {
      ...
      headlineHi: 'अलग-अलग स्थानों पर भारी बारिश की संभावना',
      ...
      actionsHi: ['30 मिनट पहले निकलें', 'अंडरपास से बचें', 'बरसाती साथ रखें'],
      validHours: 12,
    },
  },
  {
    ...
    warning: {
      ...
      source: 'IMD',
      event: 'Cyclone',
      headline: 'Cyclone warning — very severe cyclonic storm approaching coast',
      headlineHi: 'चक्रवात चेतावनी — अति भीषण चक्रवाती तूफ़ान तट की ओर',
      ...
      actionsHi: ['समुद्र में न जाएँ', 'तटीय हैं तो आश्रय जाएँ', 'फ़ोन चार्ज, पानी भरें'],
      validHours: 36,
    },
  },
  {
    key: 'beach_day',
    label: 'Beach day',
    labelHi: 'समुद्र तट दिवस',
    emoji: '🏝️',
    condition: 'sunny',
    desc: 'Calm seas, UV 9 — beach persona shines',
    params: { aqi: 45, uv: 9, precip: 0, rainProb: 10, wind: 12, wave: 0.8, waterTemp: 28, temp: 31, humidity: 66, pollen: 2 },
  },
  {
    key: 'frost_night',
    label: 'Frost night',
    ...
    warning: {
      ...
      source: 'IMD',
      event: 'Cold Wave',
      headline: 'Ground frost likely tonight in open fields',
      headlineHi: 'आज रात खुले खेतों में पाला पड़ने की संभावना',
      ...
      actionsHi: ['शाम को हल्की सिंचाई', 'नर्सरी में धुआँ/ढकाव', 'पौध ढकें'],
      validHours: 14,
    },
  },
];
...
export const DEMO_USERS: UserProfile[] = [
  {
    ...
    locations: [
      { type: 'home', label: 'Kothrud' },
      { type: 'work', label: 'Hinjewadi Ph-2' },
    ],
    city: 'pune',
    language: 'en',
    behaviorBias: { running_window: 0.9, aqi: 0.75 },
  },
  {
    id: 'ramesh',
    name: 'Ramesh',
    nameHi: 'रमेश',
    personas: ['agriculture', 'commuter'],
    conditions: [],
    activities: [
    ...
    locations: [
      { type: 'home', label: 'Kharar' },
      { type: 'farm', label: 'Wheat farm · 4 acres' },
    ],
    city: 'chandigarh',
    language: 'hi',
    behaviorBias: { farm_frost: 0.9, farm_irrigation: 0.8 },
  },
  {
    id: 'asha',
    name: 'Asha',
    nameHi: 'आशा',
    personas: ['parent', 'health', 'events'],
    conditions: ['pollen_allergy'],
    activities: [
    ...
    locations: [
      { type: 'home', label: 'Andheri West' },
      { type: 'school', label: 'Juhu' },
    ],
    city: 'mumbai',
    language: 'en',
    behaviorBias: { school_commute: 0.85, event_weather: 0.7 },
  },
];
...
export function resolveParams(s: ScenarioDef): WeatherParams {
  ...
  return {
    ...
    precip,
    rainProb: p.rainProb ?? precip * 4,
    visibility: p.visibility ?? (precip < 5 ? 10 : 3),
    uv: p.uv ?? 5,
    aqi,
    pm25: p.pm25 ?? round1(aqi * 0.6),
    wave: p.wave,
...
export function buildDaily(s: ScenarioDef, p: WeatherParams): DayPoint[] {
  ...
  const seq: Partial<Record<ScenarioKey, number[]>> = {
    rainy_commute: [85, 70, 40, 20, 15],
    cyclone: [95, 90, 60, 30, 15],
    aqi_spike: [4, 6, 10, 30, 40],
    heatwave: [8, 5, 5, 20, 35],
    frost_night: [10, 10, 5, 5, 15],
  };
...
export function aqiModel(p: WeatherParams, user: UserProfile, lang: Lang): Model {
  ...
  // piecewise map of CPCB bands → favourability score
  ...
  return {
    score,
    level: L(lang, band.label, band.labelHi),
    summary,
    factors: [
...
export function runningModel(p: WeatherParams, hourly: HourPoint[], user: UserProfile, lang: Lang, cycle = false): Model {
  ...
  // best 90-minute window between 5 and 21
...
export function irrigationModel(p: WeatherParams, daily: DayPoint[], lang: Lang): Model {
  ...
  return {
    score,
    level: dry || rainSoon ? L(lang, 'Action', 'कार्रवाई') : L(lang, 'OK', 'ठीक'),
    summary,
...
const TIME_WINDOWS: Partial<Record<CardType, [number, number][]>> = {
  running_window: [[5, 9], [16, 20]],
  cycling_window: [[5, 9], [16, 20]],
  work_commute: [[7, 10], [17, 20]],
  school_commute: [[7, 9], [14, 16]],
  event_weather: [[15, 22]],
  farm_frost: [[17, 24], [0, 7]],
  farm_irrigation: [[5, 9], [16, 19]],
  uv: [[9, 16]],
  heat_stress: [[10, 17]],
  fog_alert: [[4, 10]],
  beach_safety: [[6, 18]],
...
export function buildHomepage(o: BuildOptions): Homepage {
  ...
  const push = (type: CardType, persona: PersonaKey, title: string, m: Model | null, opts: { phase?: Phase; source?: Source; why: string; severity?: Severity; pinned?: boolean; summary?: string; data?: Record<string, unknown>; actions?: string[]; confidence?: number }) => {
    ...
    candidates.push({
      ...
      score: m?.score,
      level: m?.level,
      phase: opts.phase ?? 'derived',
      severity: opts.severity,
      bestWindow: m?.bestWindow,
      factors: m?.factors,
      actions: opts.actions ?? m?.actions,
  ...
  const pinnedCards = cards.filter((c) => c.type === 'severe_warning' && (c.severity === 'red' || c.severity === 'orange'));
  const rest = cards.filter((c) => !pinnedCards.includes(c)).slice(0, 8);
  pinned.push(...pinnedCards);
  ...
  const myDay: MyDayItem[] = user.activities.map((a) => {
    ...
    switch (a.type) {
      ...
      case 'yoga': {
        ...
        if (score < 60) {
          const rm = runningModel(p, hourly, user, lang);
          suggestion = L(lang, `Shift to ${rm.bestWindow}`, `${rm.bestWindow} पर करें`);
        }
        break;
      }
      case 'commute':
    ...
    if (scenario.warning && (scenario.warning.severity === 'red')) {
      score = Math.min(score, 15);
      suggestion = L(lang, 'Cancel — red alert in force', 'रद्द करें — रेड अलर्ट');
    }
    return { activity: a, hour: h, score, status: score >= 60 ? 'go' : score >= 40 ? 'shift' : 'avoid', note, suggestion, point: pt };
  });

  // ---- brief (template NLG, never free-form LLM) -----------------------
